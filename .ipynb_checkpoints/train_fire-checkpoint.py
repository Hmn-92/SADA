...
class MutualInformationMinimizationLoss(nn.Module):
    ...
    def forward(self, upper_feat, lower_feat):
        # 归一化特征
        upper_feat = F.normalize(upper_feat, p=2, dim=1)
        lower_feat = F.normalize(lower_feat, p=2, dim=1)

        # 计算相关性矩阵
        corr_matrix = torch.mm(upper_feat, lower_feat.T)  # [B, B]

        # 对角线屏蔽，避免同一特征之间的影响
        mask = torch.eye(corr_matrix.size(0), device=corr_matrix.device).bool()
        corr_matrix.masked_fill_(mask, 0)

        # 取均值作为互信息最小化损失
        mi_loss = torch.mean(corr_matrix)
        return mi_loss


# 在训练前定义互信息最小化损失
mi_loss_fn = MutualInformationMinimizationLoss()
...
def train(args, epoch, dataset, train_loader, model, classifier,
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        loss += class_loss
        # 互信息最小化损失
        # upper_feat, lower_feat = feat[:, 2048:4096], feat[:, 4096:]

        global_feat, upper_feat, lower_feat = feat[:, :2048], feat[:, 2048:4096], feat[:, 4096:]
        # global_feat, upper_feat, lower_feat = feat[:, :1024], feat[:, 1024:2048], feat[:, 2048:]
        # global_feat, upper_feat, lower_feat = feat[:, :3072], feat[:, 3072:6144], feat[:, 6144:]
        # upper_feat, lower_feat = feat[:, 1024:2048], feat[:, 2048:]
        # upper and lower feature
        mi_loss = mi_loss_fn(upper_feat, lower_feat)
        # # global and upper feature
        # mi_loss += mi_loss_fn(global_feat, upper_feat)
        # # global and lower feature
        # mi_loss += mi_loss_fn(global_feat, lower_feat)
